...
def optimism_with_cost_bonus(
    ...
    k_hat = jnp.ones(shape=(du, dx), dtype=a_star.dtype) * 0.01
...
def run_many(
    ...
    return jax.vmap(
        lambda key: optimism_with_cost_bonus(
            a_star=a_star,
            b_star=b_star,
            q=q,
            r=r,
...
def cec_with_pe(
    ...
    act_std = jnp.array(1.0, dtype=a_star.dtype)  # initial exploration std

    def one_step(carry, t):
        key, x, k_hat, v_t, v_t_cur, s_t, next_update, act_std = carry

        # --- exploration noise in action: std updated only at doubling points ---
        ...
        def update_branch(args):
            k_hat, v_t, next_update, act_std = args
            ...
            # update exploration std at doubling point
            act_std_new = 1.0 / jnp.sqrt(t.astype(a_star.dtype) + 1.0)
            return (k_new, v_t_new, next_update_new, act_std_new)

        def no_update_branch(args):
            return args

        k_hat, v_t, next_update, act_std = jax.lax.cond(
            do_update,
            update_branch,
            no_update_branch,
            operand=(k_hat, v_t, next_update, act_std),
        )
        ...
        carry_next = (key, x_next, k_hat, v_t, v_t_cur, s_t, next_update, act_std)
        outs = (cost, regret, do_update)
        return carry_next, outs

    init_carry = (key, x0, k_hat, v_t, v_t_cur, s_t, next_update, act_std)
    carry_final, (costs, regrets, updates) = jax.lax.scan(
        one_step,
        init_carry,
...
@partial(jax.jit, static_argnames=("num_steps",))
def run_many_doubling(a_star, b_star, q, r, keys, lam, num_steps, noise_sigma, x0):
    return jax.vmap(
        lambda key: cec_with_pe(
            a_star=a_star,
            b_star=b_star,
            q=q,
            r=r,
...
if __name__ == "__main__":
    ...
    # a_star = jnp.array([[1.0, dt], [0.0, 1.0]])
    # b_star = jnp.array([[0.5 * dt**2], [dt]])
    # q = jnp.eye(d_x)
    # r = jnp.eye(d_u)
